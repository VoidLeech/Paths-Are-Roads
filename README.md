...
Paths are Roads currently support the following mods in various versions of Minecraft. For details on which mobs and blocks are supported, check out our[wiki](https://github.com/VoidLeech/Paths-Are-Roads/wiki/Mod-Support).  
...
- Hellion's Sniffer+
- Simple Kelpies
- Macaw's Paths and Pavings (Also with limited Every Compat support, see [the issue](https://github.com/VoidLeech/Paths-Are-Roads/issues/10) for details)
- Infernal Expansion
- Environmental
- More Paths Repathed
- Undergarden Paths
...
A: Paths are Roads has full customizability w.r.t. the blocks that count as paths, the entities that they apply to, and comes with extensive built-in mod support.
...
A: Is X.x.x newer than the latest version? Then probably yes. Is it 1.17.x? Then maybe. Any other version? Then probably no. Check out our [support policy](https://github.com/VoidLeech/Paths-Are-Roads/wiki/Support-Policy) for specifics and whether a version is still maintained.

**Q: Why do your files list compatible versions even though your wiki doesn't list those versions in the table?**  
...
A: Yes, but do note our license does not allow directly including the data pack files in your pack.
